...
RegisterNetEvent('esx_advancedgarage:addVehicle')
AddEventHandler('esx_advancedgarage:addVehicle', function(netId)
    addVehicleToTable(source, netId)
end)

local function addVehicleToTable(playerId, netId)
    local xPlayer = ESX.GetPlayerFromId(playerId)
    if xPlayer then
        local identifier = xPlayer.getIdentifier()
        if not PlayerVehicles[identifier] then PlayerVehicles[identifier] = {} end
        PlayerVehicles[identifier][netId] = true
    end
end

-- Amikor a játékos betölt
AddEventHandler('esx:playerLoaded', function(playerId, xPlayer)
    ...
    SetTimeout(10000, function()
        if PlayerVehicles[identifier] then
            TriggerClientEvent('esx_advancedgarage:retoggleBlip', playerId, PlayerVehicles[identifier])
        end
    end)
end)
...
ESX.RegisterServerCallback('esx_advancedgarage:storeBoat', function (source, cb, vehicleProps)
	...
	MySQL.prepare('SELECT 1 FROM `owned_vehicles` WHERE `owner` = ? AND `type` = ? AND `job` = ? AND `plate` = ?', {xPlayer.identifier, 'boat', 'civ', vehicleProps.plate}, function(result)
		if result then
            cb(true)
        else
...
RegisterNetEvent('esx_advancedgarage:spawnVeh', function(props, coords)
	...
	ESX.GetVehicleType(vehicleModel, src, function(vehicleType)
		if vehicleType then
			local createdVehicle = CreateVehicleServerSetter(vehicleModel, vehicleType, coords.x, coords.y, coords.z + 1.1, coords.w)
			local a = 0
			while not DoesEntityExist(createdVehicle) do
				Wait(50)
				a += 1
				if a > 20 then
                    return error('[^1ERROR^7] Unfortunately, this vehicle has not spawned')
                end
			end
			...
			if Config.VehBlip then
                ...
                TriggerClientEvent('esx_advancedgarage:toggleBlip', src, id)
				addVehicleToTable(src, id)
			end
			Wait(500)
			TaskWarpPedIntoVehicle(GetPlayerPed(src), createdVehicle, -1)
		else
			error(('[^1ERROR^7] Tried to spawn invalid vehicle - ^5%s^7!'):format(vehicleModel))
		end
	end)
end)
